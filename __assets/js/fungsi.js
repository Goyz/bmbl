...
function genGrid(modnya, divnya, lebarnya, tingginya){
	...
	switch(modnya){
		case "voucher":
			...
			kolom[modnya] = [	
				{field:'nominal',title:'Nominal Poin/Hari',width:150, halign:'center',align:'right',
				...
				{field:'qty',title:'Jumlah Voucher',width:150, halign:'center',align:'right'},
				{field:'create_date',title:'DiBuat Tgl.',width:150, halign:'center',align:'center'},
				{field:'create_by',title:'DiBuat Oleh',width:250, halign:'center',align:'left'},
				{field:'flag',title:'Sistem Voucher',width:250, halign:'center',align:'center',
					formatter:function(value,rowData,rowIndex){
						if(value=='P'){
							return "Sistem Poin";
						}else{
							return "Sistem Bulanan";
						}
					},
					styler:function(value,rowData,rowIndex){
						if(value=='P'){return 'background:green;color:#ffffff'}
						else{return 'background:yellow;color:navy'}
					}
				},
				{field:'id',title:'View',width:250, halign:'center',align:'center',
					formatter:function(value,rowData,rowIndex){
						return "<a href='javascript:void(0);' onclick='cetak_voucher("+value+")'>Lihat/Cetak Voucher</a>";
					}
				}
...
function gen_editor(id){
	tinymce.PluginManager.load('equationeditor', host+'__assets/tinymce_equation/build/plugin.min.js');
	tinymce.init({
		  selector: id,
		  height: 100,
		  plugins: [
				...
				"insertdatetime media table contextmenu paste jbimages equationeditor"
		    ],
		  content_css: host+'/__assets/tinymce_equation/build/mathquill.css',
		  // ===========================================
		  // PUT PLUGIN'S BUTTON on the toolbar
		  // ===========================================
		  menubar: false,
		  toolbar: "insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image jbimages  | equationeditor",
...
function get_poin(vid,flag,flag_vid){
	$.post(host+'hitungpoin',{vid:vid,flag:flag,flag_vid:flag_vid},function(r){
		if(flag=='get'){
...
function endEditing(mod){
    if (editIndex == undefined){return true}
    if ($('#'+mod).datagrid('validateRow', editIndex)){
        $('#'+mod).datagrid('endEdit', editIndex);
        editIndex = undefined;
        return true;
    } else {
        return false;
...
function onEndEdit(index, row){
	//console.log($(this).attr('modul'));
	if($(this).attr('modul')!="ad_detil_vo"){
		var ed = $(this).datagrid('getEditor', {
			index: index,
			field: 'kode_rev_det'
		});
		row.jenis_revisi = $(ed.target).combobox('getText');
	}
	else{
		var ed = $(this).datagrid('getEditor', {
			index: index,
		...
		row.country_name = $(ed.target).combobox('getText');
	}
}
function append(mod){
    if (endEditing(mod)){
        $('#'+mod).datagrid('appendRow',{status:'add'});
        editIndex = $('#'+mod).datagrid('getRows').length-1;
        $('#'+mod).datagrid('selectRow', editIndex)
                .datagrid('beginEdit', editIndex);
    }
}
function removeit(mod){
    if (editIndex == undefined){return}
    $('#'+mod).datagrid('cancelEdit', editIndex)
            .datagrid('deleteRow', editIndex);
    editIndex = undefined;
}
function accept(mod){
    if (endEditing(mod)){
        $('#'+mod).datagrid('acceptChanges');
    }
}
function reject(){
...
function send_grid_data(id_grid){
	var data_post={};
	var data_prop=$('#'+id_grid).datagrid('getData');
	delete data_prop.total;
	data_post['config']=data_prop.rows;
	$.post(host+'simpan/tbl_config/edit',data_post,function(r){
	  //console.log(r);
		if(r==1){
			$.messager.alert('BIMBEL', 'Data Konfigurasi Tersimpan', 'info');
			$('#'+id_grid).datagrid('reload');	
		}
		else{
			console.log(r);
			$.messager.alert('BIMBEL', 'Data Failed Submitted', 'error');
		}
	});
}
...
var snapshots = [];
function capture(video, scaleFactor,uniq_id) {
    if(scaleFactor == null){
        scaleFactor = 1;
    }
	...
	//	att.value = "c_"+uniq_id;                           // Set the value of the class attribute
 ...
 */
function shoot(id_video,uniq_id,output_id){
    var video  = document.getElementById(id_video);
    var output = document.getElementById(output_id);
	console.log(video);
	//console.log(output_id);
    var canvas = capture(video, scaleFactor,uniq_id);
...
function cetak_voucher(id){
	openWindowWithPost(host+'CetakVoucher',{id:id});
}