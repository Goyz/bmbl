...
class lib {
	...
	function kirimemail($type="", $email="", $p1="", $p2="", $p3=""){
		...
		switch($type){
			...
			case "email_notif":
				...
				$html = "
					...
					<br/>
					<br/>
					<h1>Aktivasi USER Bimbel Online</h1>
					<br/>
					...
					<br/>
					<br/>
					<a href='".$ci->config->item('base_url')."aktivasi/".$p1."'>aktivasi</a>
				";
			break;
		}
		//echo $html;exit;
		...
		try {
		if($ci->config->item('SMTP')) $ci->phpmailer->IsSMTP();                     
		$ci->phpmailer->SMTPAuth   = $ci->config->item('SMTPAuth');       
		$ci->phpmailer->SMTPSecure = $ci->config->item('SMTPSecure');
		$ci->phpmailer->Port       =  $ci->config->item('Port');
		
		$ci->phpmailer->Host       = $ci->config->item('Host'); 
		$ci->phpmailer->Username   = $ci->config->item('Username');     
		$ci->phpmailer->Password   = $ci->config->item('Password');            

		$ci->phpmailer->AddReplyTo($ci->config->item('EmaiFrom'),$ci->config->item('EmaiFromName'));

		$ci->phpmailer->From       = $ci->config->item('EmaiFrom');
		$ci->phpmailer->FromName   = $ci->config->item('EmaiFromName');
		$ci->phpmailer->AddAddress($email);
		
		//$ci->phpmailer->AddCC('amatin@scisi.com', 'Amatin');
		$ci->phpmailer->AddCC('yogi_p4try4@yahoo.com', 'Goyz');
		//$ci->phpmailer->AddCC('hasan.abd@scisi.com', 'Hasan');
		$ci->phpmailer->Subject   = $subject;
		$ci->phpmailer->AltBody   = "To view the message, please use an HTML compatible email viewer!"; 
		$ci->phpmailer->WordWrap  = 100; 
		$ci->phpmailer->MsgHTML($html);
		$ci->phpmailer->IsHTML(true);
		
		if($ci->phpmailer->Send())return 1;
		else return $ci->phpmailer->print_debugger();
		}catch (phpmailerException $e) {
			return $e->errorMessage();
		}
		/*
		$ci->phpmailer->initialize($config);
		$ci->phpmailer->from("triwahyunugros@gmail.com");
		$ci->phpmailer->to($email);
		$ci->phpmailer->subject($subject);
		$ci->phpmailer->message($html);
		$ci->phpmailer->set_newline("\r\n");
		if($ci->phpmailer->send())
			//echo "<h3> SUKSES EMAIL ke $email </h3>";
			return 1;
		else
			//echo $ci->email->print_debugger();
			return $ci->phpmailer->print_debugger();
		*/
	}	
