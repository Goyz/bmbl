...
class mhome extends CI_Model{
	...
	function getdata($type="", $p1="", $p2="",$p3="",$p4=""){
		...
		switch($type){
			case "cek_voucher":
				...
				$data['res']='';
				$sql="SELECT A.*,B.nominal,B.flag as sys_poin 
						FROM tbl_voucher_detil A 
				...
				if(isset($ex['kode_voucher'])){
					if($ex['flag']=='A'){
						$data['msg']=$this->isi_voucher($p1,$ex['nominal'],$ex['sys_poin']);
					}else{
						$data['msg']=2;
					}
				}
			...
			case "get_poin":
				...
				$sql="SELECT DATEDIFF(A.tgl_berakhir,CURDATE())AS sisa_hari
						FROM tbl_poin_bulanan A 
						WHERE A.cl_peserta_id=".$this->auth['peserta_id']." 
						AND A.flag='A'";
				$data['poin_bulanan']=$this->db->query($sql)->row_array();
				if(isset($data['poin_bulanan']['sisa_hari'])){
					if($data['poin_bulanan']['sisa_hari'] < 0){$data['poin_bulanan']['sisa_hari']='Tidak Ada';}
				}
				$sql="SELECT A.*, 
						CASE 
			...
			case "cek_bulanan":
				$poin=-1;
				$sql="SELECT DATEDIFF(tgl_berakhir,CURDATE())as sisa 
						FROM tbl_poin_bulanan 
						WHERE cl_peserta_id=".$this->auth['peserta_id']." AND flag='A'";
				$res=$this->db->query($sql)->row_array();
				if(isset($res['sisa'])){$poin=$res['sisa'];}
				return $poin;
			break;
			case "get_video":
	...
	function calc_poin(){
		...
		$msg['msg']=0;
		$msg['poin']=0;
		$cek_bulanan=$this->getdata('cek_bulanan');
		if($cek_bulanan < 0){
			$this->db->trans_begin();
	...
	function isi_voucher($voucher,$nominal,$sys){
		$this->db->trans_begin();
		...
		if($this->db->insert('tbl_voucher_pakai',$post)){
			if($sys=='P'){
				...
				$sql="SELECT DATE_ADD(CURDATE(), INTERVAL ".$nominal." DAY) as tgl_berakhir";
				$tgl_berakhir=$this->db->query($sql)->row('tgl_berakhir');
				$sql="SELECT * FROM tbl_poin_bulanan WHERE cl_peserta_id=".$this->auth['peserta_id']." AND flag='A'";
				$ex=$this->db->query($sql)->row_array();
				
				if(isset($ex['id'])){
					$sql="UPDATE tbl_poin_bulanan SET flag='T' WHERE id=".$ex['id'];
					$this->db->query($sql);
				}
				$data_poin=array('cl_peserta_id'=>$this->auth['peserta_id'],
				...
				$this->db->insert('tbl_poin_bulanan', $data_poin);
